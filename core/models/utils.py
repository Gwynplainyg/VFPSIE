...
import sys
sys.path.append('.')
import core.models.softsplat as softsplat


def warp(img, flow, direction="forward"):
    if direction== "forward":
        return forward_warp(img,flow)
    else:
        return backward_warp(img,flow)
...
def backward_warp(img,flow):
    ...
    mask = (grid_[:, :, :, 0] < W) & (grid_[:, :, :, 0] > 0) & (grid_[:, :, :, 1] < H) & (grid_[:, :, :, 1] > 0)
    return output, mask
